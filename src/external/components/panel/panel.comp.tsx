...
export const Panel = (props: PanelProps) => {
  ...
  const [panelActive, setPanelActive] = useState<boolean>(openedByDefault || false);
  const [height, setHeight] = useState<number>(panelActive ? content?.current?.scrollHeight : 0);
  ...
  useEffect(() => {
    ...
    if (!panelActive) {
      ...
      if (accordionMode && !openedByDefault) {
        // Close other panels
        const openedPanels = panelRef.current.parentElement.querySelectorAll('.active.visible');
  ...
  return (
    <div 
      className={`panel ${panelActive ? 'active' : ''} ${isVisible ? 'visible' : ''}`} 
      {...(panelProps || {})}
      ref={panelRef}
    >

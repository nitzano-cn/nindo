...
class EventService extends HttpService {
  ...
  public reportEvent(groupType: EventGroupType, eventType: ClientEvent | EngagementEvent, compId: string | null, eventData?: any) {
    ...
    if (groupType === EventGroupType.CLIENT || groupType === EventGroupType.PERFORMANCE) {
    ...
    if (eventData && typeof eventData === 'object') {
      for (const key in eventData) {
        const value: string = typeof eventData[key] === 'object' ? JSON.stringify(eventData[key]) : eventData[key];
        otherQueryParams += `&${key}=${value}`;
      }
    }

    this.makeRequest(`${baseUrl}/api/v1/event/report/${compId}?groupType=${groupType}&eventType=${eventType}${otherQueryParams}`, {}, false).catch(() => {});
  }
  ...
  private reportEngagementEvent(compId: string, data: any) {
    ...
    this.reportEvent(EventGroupType.ENGAGEMENT, data.eventType, compId, eventData);
  }
  ...
  public postEventToParent(eventName: TEventName, compId: string | null, eventData?: any) {
    if (!compId) {
      return;
    }

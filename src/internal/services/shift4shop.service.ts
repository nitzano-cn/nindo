...
class Shift4ShopService extends HttpService {
  ...
  public async getForEditor(pluginId: string = '', defaultPluginData?: IPlugin<any>) {
    const url = `${apiBaseUrl}/api/v1/shift4shop/${this.pluginType}/${pluginId}?serviceName=${this.serviceName}&${this.queryParams}`;
  ...
  public async create(body: any) {
    return await this.makeRequest(`${apiBaseUrl}/api/v1/shift4shop/${this.pluginType}?serviceName=${this.serviceName}&${this.queryParams}`, {
      method: 'post',
      headers: {
  ...
  public async update(pluginId: string = '', body: IPlugin<any>) {
    return await this.makeRequest(`${apiBaseUrl}/api/v1/shift4shop/${this.pluginType}/${pluginId}?serviceName=${this.serviceName}&${this.queryParams}`, {
      method: 'put',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const shift4ShopService = new Shift4ShopService();
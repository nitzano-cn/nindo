...
class BigCommerceService extends HttpService {
  ...
  public async getForEditor(pluginId: string = '', defaultPluginData?: IPlugin<any>) {
    const url = `${apiBaseUrl}/api/v1/bigcommerce/${this.pluginType}/${pluginId}?serviceName=${this.serviceName}&${this.queryParams}`;
  ...
  public async create(body: any) {
    return await this.makeRequest(`${apiBaseUrl}/api/v1/bigcommerce/${this.pluginType}?serviceName=${this.serviceName}&${this.queryParams}`, {
      method: 'post',
      headers: {
  ...
  public async update(pluginId: string = '', body: IPlugin<any>) {
    return await this.makeRequest(`${apiBaseUrl}/api/v1/bigcommerce/${this.pluginType}/${pluginId}?serviceName=${this.serviceName}&${this.queryParams}`, {
      method: 'put',
      headers: {
        'Content-Type': 'application/json'
      },

...
class PlansService extends HttpService {
  ...
  async getUserSubscription(serviceName: string, vendor?: string) {
    ...
    return await this.makeRequest(`${apiBaseUrl}/api/subscription${vendor ? '/v/' + vendor : ''}/${serviceName}${query}`);
  }

  async reportFunnelEvent(eventType: string) {
    return await this.makeRequest(`${apiBaseUrl}/api/funnelEvent?type=${eventType}`, {}, false);
  }

...
class ExportService extends HttpService {
  ...
  async createExportJobForUrl(params: { url: string | null, format: 'image' | 'pdf', selector?: string, selectorType?: string, delay?: number, width?: number, height?: number, hideElements?: string[] }) {
    const componentType = this.pluginType;
    const { url, format, selector, selectorType, delay, width, height, hideElements } = params;
    if (!url) {

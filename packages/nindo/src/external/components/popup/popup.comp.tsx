...
export const Popup = (props: IPopupProps) => {
	const { className, style, show, closeCallback, children, refCallback } =
		props;
		...
		try {
			if (typeof window !== 'undefined' && window !== null) {
				if (
					...
					window?.self.innerHeight > window?.top?.innerHeight
				) {
					const scrollTop =
	...
	return show ? (
		<PortalPopup className={`popup ${className || ''}`}>
			<div
				className={`overlay ${fadedIn ? 'faded-in' : ''}`}
				onClick={(e: any) => closeCallback(e)}
			></div>
			<div
				className={`popup-content ${fadedIn ? 'faded-in' : ''}`}
				style={popupStyle}
				ref={refCallback}
			>
				{children}

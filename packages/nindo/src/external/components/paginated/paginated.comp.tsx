...
type PaginatedProps = {
	...
	itemsRenderer: (
		...
		pages: number,
		total: number
	) => ReactChild | ReactElement | JSX.Element[] | JSX.Element | string;
	loaderRenderer?:
		| ReactChild
		| ReactElement
		| JSX.Element[]
		...
		| string;
	paginationRenderer?: (
		paginationState: IPaginationResponse,
		queryParams: IPaginationQueryParams,
		setQueryParams: (params: IPaginationQueryParams) => void
	) => ReactChild | ReactElement | JSX.Element[] | JSX.Element | string;
	errorRenderer?: (
		message: string
	) => ReactChild | ReactElement | JSX.Element[] | JSX.Element | string;
...
export const Paginated = forwardRef((props: PaginatedProps, ref: any) => {
	...
	const [paginationState, setPaginationState] = useState<IPaginationResponse>({
		limit,
		docs: [],
		page: 1,
		pages: 0,
	...
	async function getItems(refresh = false) {
		...
		try {
			...
			const result: IHttpResult = await httpService.makeRequest(
				`${endpoint}?limit=${limit}&page=${page}&q=${q}&sortBy=${sortBy}${
		...
		return (
			...
			(paginationRenderer ? (
				...
				<div className="pagination">
					...
					<span>
						Page {queryParams.page}/{paginationState.pages}
					</span>
					<Button
						color="transparent"
						size="small"
						disabled={queryParams.page >= paginationState.pages}
						onClick={() =>
							setQueryParams({ ...queryParams, page: queryParams.page + 1 })
						}
					>

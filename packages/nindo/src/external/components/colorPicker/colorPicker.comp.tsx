  ...
  setElmPosition = () => {
    ...
    switch (position) {
      case 'top':
        ...
        break;
      case 'bottom':
        this.el.style.top = `${scrollTop + bounding.top + bounding.height + spacing}px`;
        this.el.style.left = `${bounding.left + (bounding.width / 2)}px`;
...
export const ColorPicker = (props: ColorPickerProps) => {
  const { selectedColor, onChange, position, showUndo, disabled, enableTransparency, defaultColor, colorFormat } = props;
  ...
  return (
    <div className="color-picker-wrapper">
      <div 
        className={`selected-color ${!selectedColor || !selectedColor.length ? 'none' : ''}`} 
      ...
      }
      {
        ...
        <div className="undo" title="Restore to default" onClick={() => colorChanged(defaultColor || '')}>

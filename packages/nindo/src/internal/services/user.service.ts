...
class UserService extends HttpService {
	async getUserInfo() {
		return await this.makeRequest(
			`${apiBaseUrl}/api/v1/user?serviceName=${this.serviceName}`
		);
	}

	async getPlanFeatures(pluginId: string = '') {
		return await this.makeRequest(
			`${apiBaseUrl}/api/v1/user/planFeatures/${pluginId}?serviceName=${this.serviceName}`
		);
	}
	...
	async logout() {
		...
		return await this.makeRequest(
			`${apiBaseUrl}/api/v1/user/logout?serviceName=${this.serviceName}`,
			{},
			false
		);
	}

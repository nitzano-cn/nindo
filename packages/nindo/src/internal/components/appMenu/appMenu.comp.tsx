	...
	return (
		<aside className="main-menu">
			<div className="inner">
				{links.map((link) => (
					<NavLink
						exact={typeof link.exact === 'undefined' ? true : link.exact}
						to={`${link.url}?${query.toString()}`}
						activeClassName="active"
						key={`menu_link_${link.name}`}
					>
						{link.icon && <FontAwesomeIcon icon={link.icon} />}
						<span>
							{link.name}
							{typeof link.badge !== 'undefined' && (
								<strong className="badge" title={`${link.badge}`}>
				...
				<div
					...
					onClick={(e: any) => {
						eventService.reportMixpanelEvent('Upgrade to premium', {

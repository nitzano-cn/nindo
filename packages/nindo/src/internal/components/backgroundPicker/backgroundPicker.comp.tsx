	...
	return (
		<>
			<div className="background-picker-wrapper">
				<div
					className={`backgorund-colors-container ${
						isPalleteOpen ? 'opened' : ''
					}`}
				>
					<div
						className={`background-color-picker white  ${
							selectedColor === '#fff' ? 'active' : ''
						}`}
						onClick={(e) => selectColor(e, '#fff')}
					></div>
					<div
						className={`background-color-picker black ${
							selectedColor === '#000000' ? 'active' : ''
						}`}
						onClick={(e) => selectColor(e, '#000000')}
					></div>
					<div
						className={`background-color-picker gray ${
							selectedColor === '#bababa' ? 'active' : ''
						}`}
						onClick={(e) => selectColor(e, '#bababa')}
					></div>
					<div
						className={`transparent-backgorund ${
				...
				<div
					className={`brush-icon-container ${isPalleteOpen && 'active'}`}
					onClick={(e) => handlePalleteClick(e)}
				>

              ...
              <>
                <div onClick={() => setActiveCycleId(idx)} className="test">
                  {idx}
					...
					<h4 className={`${activeCycleId === 0 ? 'active' : ''}`}>
						{planData.cycles[0].period}ly
					</h4>
					<label className="switch">
						<input
							type="checkbox"
							onChange={(e) => setActiveCycleId(e.target.checked ? 0 : 1)}
							checked={activeCycleId === 0 ? true : false}
						/>
						<span className="slider"></span>
					</label>
					<h4 className={`${activeCycleId === 1 ? 'active' : ''}`}>
						{planData.cycles[1].period}ly
					</h4>
				</div>
			</div>

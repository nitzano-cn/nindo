...
function updateComponentTypes(components) {
  const content = `
export type TComponentType = ${components.map(comp => (`'${comp.name}'`)).join(' | ')};
...
export const pluginsList: IPluginListing[] = ${JSON.stringify(components)} as IPluginListing[];
  `.trim();
  ...
  try {
    ...
    fs.writeFileSync('./dist/types/external/types/component.types.d.ts', content.replace(/export /g, 'export declare '));
...
${components.map(comp => {
  ...
  try {
    const tsRes = tsCompile(content);
    fs.writeFileSync('./dist/types/external/types/icon.types.d.ts', content.replace(/export /g, 'export declare '));
    fs.writeFileSync('./dist/types/external/types/icon.types.js', tsRes);
  } catch(e) {}
}

https.get('https://www.commoninja.com/api/v1/plugin/types', res => {
  let data = [];
